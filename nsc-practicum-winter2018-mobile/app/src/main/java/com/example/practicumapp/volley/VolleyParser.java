...
public class VolleyParser {
        ...
        MyVolleySingleton.getInstance(context)
                .sendObjectRequest(accessToken, Request.Method.POST, urlWithParams, parameters, new VolleyResponseListener() {
                    ...
                    public void onSuccess(JSONObject response) {
                        User newUser = getUserObject(response);
                        if (newUser.getType().toLowerCase().equals("employee")) {
                            String newUserName = newUser.getFirstName() + " " + newUser.getLastName();
                            assignEmployeeToManager(newHire.getManager(), newUser.getActiveDirectoryID(), newUserName, volleyUserResponseListener);
                        } else {
                            volleyUserResponseListener.onSuccess(newUser);
                        }
                    }
                });
    }
     ...
     * @param volleyUserResponseListener Listener to callback back on success
     */
     ...
     */
    private User getUserObject(JSONObject object) {
        try {
